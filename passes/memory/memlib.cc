		...
		for (auto opts: make_opt_combinations(ram.opts)) {
			...
			for (auto &pdef: ram.ports) {
				if (!opts_ok(pdef.opts, opts))
					continue;
				PortGroup grp;
				grp.optional = find_single_cap(pdef.val.optional, opts, Options(), "optional");
				grp.optional_rw = find_single_cap(pdef.val.optional_rw, opts, Options(), "optional_rw");
				grp.names = pdef.val.names;
				for (auto portopts: make_opt_combinations(pdef.val.portopts)) {
					bool forbidden = false;
					for (auto &fdef: ram.forbid) {
						if (opts_ok(fdef.opts, opts) && opts_ok(fdef.portopts, portopts)) {
							forbidden = true;
						}
					}
					if (forbidden)
					...
					var.kind = pdef.val.kind;
					if (pdef.val.kind != PortKind::Ar) {
						const ClockDef *cdef = find_single_cap(pdef.val.clock, opts, portopts, "clock");
						if (!cdef)
						...
						var.clk_en = find_single_cap(pdef.val.clken, opts, portopts, "clken");
					}
					const PortWidthDef *wdef = find_single_cap(pdef.val.width, opts, portopts, "width");
					...
					if (pdef.val.kind == PortKind::Srsw || pdef.val.kind == PortKind::Sr) {
						const RdWrKind *rdwr = find_single_cap(pdef.val.rdwr, opts, portopts, "rdwr");
						var.rdwr = rdwr ? *rdwr : RdWrKind::Undefined;
						var.rd_en = find_single_cap(pdef.val.rden, opts, portopts, "rden");
						const ResetValKind *iv = find_single_cap(pdef.val.rdinit, opts, portopts, "rdinit");
						var.rdinitval = iv ? *iv : ResetValKind::None;
						const ResetValKind *arv = find_single_cap(pdef.val.rdarst, opts, portopts, "rdarst");
						var.rdarstval = arv ? *arv : ResetValKind::None;
						const SrstDef *srv = find_single_cap(pdef.val.rdsrst, opts, portopts, "rdsrst");
						if (srv) {
					...
					var.wrbe_separate = find_single_cap(pdef.val.wrbe_separate, opts, portopts, "wrbe_separate");
					if (var.wrbe_separate && cram.byte == 0) {
						log_error("%s:%d: `wrbe_separate` used without `byte`.\n", filename.c_str(), orig_line);
					}
					for (auto &def: pdef.val.wrprio) {
						if (!opts_ok(def.opts, opts))
							continue;
						if (!opts_ok(def.portopts, portopts))
							continue;
						var.wrprio.push_back(port_ids.at(def.val));
					}
					for (auto &def: pdef.val.wrtrans) {
						if (!opts_ok(def.opts, opts))
							continue;
						if (!opts_ok(def.portopts, portopts))
							continue;
