...
struct MemConfig {
	// Index of the Ram in the Library.
	const Ram *def;
...
Swizzle MemMapping::gen_swizzle(const MemConfig &cfg, int sw_wide_log2, int hw_wide_log2) {
	...
	int addr_shift = cfg.def->abits - cfg.base_width_log2 + GetSize(emu_wide_bits);
...
void MemMapping::emit(const MemConfig &cfg) {
	...
	Swizzle init_swz = gen_swizzle(cfg, 0, GetSize(cfg.def->dbits) - 1);
